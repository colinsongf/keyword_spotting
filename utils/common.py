...
def moving_average(array, n=5, padding=False):
    if n % 2 == 0:
        raise Exception('n must be odd')
    if len(array.shape) != 2:
        raise Exception('must be 2-D array.')
    if n > array.shape[1]:
        raise Exception('n larger than array length')
    # array is 2D array
    if padding:
        pad_num = n / 2
        array = np.pad(array=array, pad_width=((0, 0), (pad_num, pad_num)), mode='constant', constant_values=0)
    array = np.cumsum(array, 1, dtype=np.float32)
